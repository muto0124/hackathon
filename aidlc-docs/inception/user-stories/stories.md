...
- Given ユーザーの予定、天気、好み、予算が登録されている, when 朝の生成時刻になる, then T-AI-DA はその日の低リスクな生活プランを提示する。
- Given 複数候補が存在する, when プランを提示する, then 比較リストではなく一つの推奨案を主表示する。
- Given プランに買い物や移動が含まれる, when 結果を表示する, then 準備に必要なリスト、店舗候補、ルート候補を添える。
...
- Given 推奨案を返す, when ユーザーが理由を確認する, then 好み、予算、天気、時間帯に基づく短い理由を表示する。
- Given 外食または購入が必要な案である, when 結果を表示する, then 店舗候補、移動候補、注文リンク候補などの準備支援を添える。
...
- Given 気温差や雨の可能性がある, when 服装を提示する, then 傘や羽織りなど低リスクな準備物も添える。
- Given 予定情報が不足している, when 服装を提示する, then 不足を明示した上で安全側の一般案を提示する。
...
- Given 予算、好み、生活圏が登録されている, when 買い物が必要な生活プランを生成する, then T-AI-DA は低リスクな買い物候補を提示する。
- Given 買い物候補を提示する, when 金額や影響が高リスク条件に該当する, then 自動決定せず承認要求または対象外にする。
- Given 買い物候補が低リスクである, when 結果を表示する, then 買い物リスト、店舗候補、注文リンク候補を添える。
...
- Given 入力が曖昧である, when 文脈から判断できる, then T-AI-DA は登録済みの好み、予定、予算を使って補完する。
...
- Given 高リスク判断を制限する, when ユーザーに表示する, then 何が制限対象かを短く説明する。
- Given 将来拡張で承認フローを導入する, when 承認が必要な判断である, then ユーザーの明示承認なしに実行準備を進めない。
...
- Given T-AI-DA が低リスクな判断を返す, when 準備支援が可能である, then 買い物リスト、店舗候補、ルート候補、注文リンク候補を提示する。
- Given 準備支援が外部サービス連携を必要とする, when MVP で未連携である, then 実行ではなく候補提示に留める。
- Given 準備候補が複数ある, when 表示する, then 主推奨を一つに絞り、補助情報として理由を添える。
